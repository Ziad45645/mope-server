...
public class Meerkat extends Animal {
    ...
    public Meerkat(int id, double x, double y, AnimalInfo info, String playerName, GameClient client) {
    ...
    private Timer spawner = new Timer(200);
    ...
    public void hidingUpd() {
        ...
        if (this.isDiving) {
            spawner.update(Constants.TICKS_PER_SECOND);
            if(spawner.isDone()){
            DirtSpot dirtSpot = new DirtSpot(this.getClient().getRoom().getID(), this.getX(), this.getY(), this.getRadius(),this.getClient().getRoom());
            this.getClient().getRoom().addObj(dirtSpot);
            spawner.reset();
            }
