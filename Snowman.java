...
public class Snowman extends Animal {
    public Snowman(int id, double x, double y, AnimalInfo info, String playerName, GameClient client) {
    ...
    public void writeCustomData_onAdd(MsgWriter writer) {
        ...
        writer.writeBoolean(false); // is camouflage
        writer.writeUInt16(1000); // carrot alpha
    }
    ...
    public void useAbility() {
        ...
        if(!this.isDiveMain() && !this.isDiveActive()) {
            if(abil.isPossible() && !abil.isRecharging() && abil.isUsable() && !this.isRunning) {
                YetiRoar roar = new YetiRoar(this.getClient().getRoom().getID(), this.getX(), this.getY(), (int) Math.floor(this.getRadius()*3), this.getClient().getRoom(), this.getAngle(), this, this.getInfo().getAnimalSpecies());
                this.getClient().getRoom().addObj(roar);
                int ang = -30;
                for(int i = 0; i < 2; i++) {
                    ...
                    Snowball snowball = new Snowball(this.getClient().getRoom().getID(), this.getX()+x, this.getY()+y, 20, this.getClient().getRoom(), this.getAngle(), 10, this, 0);
                    ang += 60;
                ...
                for(int i = 0; i < 3; i++) {
                    ...
                    Snowball snowball = new Snowball(this.getClient().getRoom().getID(), this.getX()+x, this.getY()+y, 20, this.getClient().getRoom(), tangle+ang, 10, this, 3);
                    ang += 60;
                    this.getClient().getRoom().addObj(snowball);
                }
                this.isRunning = true;
