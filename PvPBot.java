...
public class PvPBot extends GameClient {
    private Animal player;
    private Room room;
    private String playerName;
    private boolean notWelcome;

    public PvPBot(Room room, String nick) {
        super(room, null);
        this.makeBot(true);
        this.room = room;
        ...
        if(this.getPlayer() != null) {
            if(!notWelcome) {
                ...
                if(welcomeTimer.isDone()) {
                    ...
                    this.room.sendChat(this, "I am pvp bot!");
                    this.room.sendChat(this, "Use your PVP button and hover it to me to 1v1 me!");
                    notWelcome = true;
                    welcomeTimer.reset();
                }
            }
            if(this.getPlayer().inArena) {
                ...
                if(moveTimer.isDone()) {
                    ...
                    if(enemy != null) {
                        ...
                        double enemyTailX = enemyX-((Math.cos(Math.toRadians(enemy.getAngle())) * (enemy.getRadius())));
                        double enemyTailY = enemyY-((Math.sin(Math.toRadians(enemy.getAngle())) * (enemy.getRadius())));
                        double enemyMouthX = enemyX+((Math.cos(Math.toRadians(enemy.getAngle())) * (enemy.getRadius())));
                        double enemyMouthY = enemyY+((Math.sin(Math.toRadians(enemy.getAngle())) * (enemy.getRadius())));
                        if(Utils.distance(this.getPlayer().getX(), enemyTailX, this.getPlayer().getY(), enemyTailY) > this.getPlayer().getRadius()/2) {
