...
public class BearSlash extends Ability {
    ...
    public BearSlash(int id, double x, double y, int radius, Room room, double angle, Animal owner, int species) {
        super(id, x, y, 62, 25, 0);
        this.room = room;
        this.owner = owner;
        ...
        if(owner instanceof PolarBear) this.specType2 = 1;
    }
    ...
    public void onCollision(GameObject o) {
        ...
        if (o instanceof Animal && !damaged.contains((Animal) o) && ((Animal) o) != this.owner && !((Animal) o).inArena
            ...
            ((Animal) o).setStun(2);
            ((Animal)o).getClient().send(Networking.personalGameEvent(255, "Oh no! You've been slashed by a Bear!"));
    ...
    public void update() {
        ...
        if (liveTimer.isDone()) {
            if(owner != null && owner instanceof Bear){
                ((Bear)owner).getInfo().getAbility().setActive(false);
                }
                if(owner != null && owner instanceof PolarBear){
                    ((PolarBear)owner).getInfo().getAbility().setActive(false);
                }
