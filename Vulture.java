...
public class Vulture extends Animal {
    public Vulture(int id, double x, double y, AnimalInfo info, String playerName, GameClient client) {
        ...
        if(this.running){
             if (o instanceof Animal) {
                        if (!((Animal) o).pickedByBird && !((Animal) o).flag_flying
                                && !((Animal) o).isStunned()) {
                            if(this.getInfo().getSkin() == 0)
                                return;

                      
                                if(this.getSpecies() != 1 && ((Animal)o).getInfo().getTier() > this.getInfo().getTier())
                                return;
    ...
    public void update() {
        ...
        if (this.running && !this.pickedByBird && !isRammed) {
            this.setSpecies(1);
        } else {
          
            if (this.flag_flying && !this.pickedByBird && isRammed)
                this.setSpecies(2);
            else
                this.setSpecies(0);
